...
test("the standalone parent owns ordered care lifecycle boundaries", () => {
  ...
  assert.match(page, /kind: "contact-loss-at-release"/);
  assert.match(page, /kind: "reduce-at-release"/);
  ...
  assert.doesNotMatch(page, /key=\{careStore\.state\.runId\}|key=\{careState\.runId\}/);
});
...
test("responsive, low-vision, and forced-color rules remain explicit source contracts", () => {
